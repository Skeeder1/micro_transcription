...
def get_resource_content(resource_path: str) -> str:
    """Load CSS or JS file content from the static directory."""
    base_dir = Path(__file__).parent / "transcription_app"
    file_path = base_dir / resource_path.lstrip('/')

    # If not found, try from current directory
    if not file_path.exists():
        file_path = Path("transcription_app") / resource_path.lstrip('/')
...
def get_html_template(sse_port: int) -> str:
    ...
    # If not found, try from current directory (for backward compatibility)
    if not template_path.exists():
        template_path = Path("transcription_app") / "templates" / "visualizer.html"
    ...
    css_content = get_resource_content("static/css/visualizer.css")
    js_content = get_resource_content("static/js/visualizer.js")
